  ...
  return (
    <div className="flex flex-col justify-between bg-white rounded-xl border overflow-hidden p-4 max-w-[350px] relative " >
      ...
      <div>
        ...
        <p className="text-sm text-gray-600">Questions <span className="font-semibold">{questions}</span></p>
        <div className="flex justify-between items-center mt-4 text-gray-500 text-sm">
          <button className="border border-purple-600 w-full text-purple-600 rounded-sm py-1 flex items-center justify-center font-semibold" onClick={hanleQuizJoin}>{loading?<div className="w-5 h-5 border-4 border-t-purple-600 border-b-transparent border-l-transparent border-r-transparent rounded-full animate-spin"></div> : "Join Quiz"}</button>
        </div>
      </div>
    </div>
  );
